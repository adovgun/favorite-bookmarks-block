...
const {
  state
} = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.store)("favorite-bookmarks-block", {
  state: {
    allBookmarks: []
  },
  actions: {
  ...
  callbacks: {
    init: () => {
      ...
      if (context.postId && state.allBookmarks.includes(context.postId)) {
        (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_0__.store)("favorite-bookmarks-block").actions.toggle();
      }
    },
    initReadingList: () => {
      // Get all bookmarks from local storage or an empty array.
